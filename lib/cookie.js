...
class Cookie {
  ...
  async get(email, password) {
    ...
    if (obj[email] && obj[email]['token']) {
      ...
      return `${Cookie.TOKENKEY}=${obj[email]['token']}`
    } else {
      await this._reflushCookie(email, password)
      obj = this._readFileObject()
  ...
  _cookieExpired(obj) {
    if (obj.Expires) {
      const expires = moment.utc(obj.Expires)
      ...
      if (error.response.status === 302) {
        ...
        if (cookie) {
          ...
          obj[email] = {
            ...
            // Expires: moment.utc(tokenObj.Expires) || ''
            Expires: moment().add(1, 'days') // expire at one day
          }
